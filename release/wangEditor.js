...
UploadVideo.prototype = {
    ...
    insertLinkVideo: function insertLinkVideo(link) {
        ...
        editor.cmd.do('insertHTML', '<video src="' + link + '" style="max-width:250px;" controls autobuffer/>');
    },
        ...
        if (uploadImgServer && typeof uploadImgServer === 'string') {
            ...
            xhr.onreadystatechange = function () {
                ...
                if (xhr.readyState === 4) {
                    ...
                    result = xhr.responseText;
                    if ((typeof result === 'undefined' ? 'undefined' : typeof result === 'undefined' ? 'undefined' : _typeof(result)) !== 'object') {
                        try {
                            result = JSON.parse(result);
                        } catch (ex) {
                            // hook - fail
            ...
            if (hooks.before && typeof hooks.before === 'function') {
                var beforeResult = hooks.before(xhr, editor, resultFiles);
                if (beforeResult && (typeof beforeResult === 'undefined' ? 'undefined' : typeof beforeResult === 'undefined' ? 'undefined' : _typeof(beforeResult)) === 'object') {
                    if (beforeResult.prevent) {
                        // 如果返回的结果是 {prevent: true, msg: 'xxxx'} 则表示用户放弃上传
                        this._alert(beforeResult.msg);
                        return;
